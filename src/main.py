...
def main():
    ...
    SurveyProcessingPipeline(source=LocalSpreadsheetSource(path=survey_path),
        schema_maker=SchemaMaker(), survey_transformer=SurveyTransformer(),
        survey_extractor=SurveyExtractor(ExcelReader())).execute()
